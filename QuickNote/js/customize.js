...
window.addEventListener("DOMContentLoaded", function () {
  ...
  chrome.storage.sync.get(null, function (storage) {
    const height =
      (storage.textarea && storage.textarea.height) || TEXTAREA.height;
    const width =
      (storage.textarea && storage.textarea.width) || TEXTAREA.width;
    textareaHeightElement.value = height;
    textareaWidthElement.value = width;
  });

  textareaHeightElement.addEventListener(
    "input",
    saveSizeOfTextarea.bind(null, "height", "width")
  );

  textareaWidthElement.addEventListener(
    "input",
    saveSizeOfTextarea.bind(null, "width", "height")
  );
  ...
  function saveSizeOfTextarea(axis1, axis2, e) {
    chrome.storage.sync.get("textarea", function (storage) {
      ...
      data[axis2] =
        (storage.textarea && storage.textarea[axis2]) || TEXTAREA[axis2];
